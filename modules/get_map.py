...
def getMap(index, coordinates, MAP_STYLE, ZOOM, tmpdir, slope=False):
    ...
    map_image = cropBorders(
        nwLat, nwLon, lon1, lat1, s_pixel, pix_w, pix_h, output_image
    )

    if slope:
        download_tiles(x1, x2, y1, y2, ZOOM, slope_tiles, tiles_dir, slope=True)
        output_image, tile_size = stitchTiles(
            x1, x2, y1, y2, ZOOM, tiles_dir, slope=True
        )
        s_pixel = ECF * math.cos(latitude) / (2.0**ZOOM * 512)
        pix_w = int(WIDTH_METERS / s_pixel)
        pix_h = int(HEIGHT_METERS / s_pixel)
        slope_image = cropBorders(
            nwLat, nwLon, lon1, lat1, s_pixel, pix_w, pix_h, output_image
        )
        ...
        slope_resized = slope_rgba.resize(
            map_rgba.size, resample=Image.Resampling.LANCZOS
        )
...
def getMetersFromCoordinates(north, south, east, west):
    widthMeters = (
        (east - west) * (ECF * math.cos(math.radians((north + south) / 2))) / 360
    )
...
def label(image, s_pixel, tile_size, index):
    ...
    draw.line(
        (10, height - 10, 10 + pix1, height - 10), fill=(0, 0, 0), width=line_width
    )
    draw.line((10, height - 15, 10, height - 5), fill=(0, 0, 0), width=line_width)
    draw.line(
        (10 + pix1, height - 15, 10 + pix1, height - 5),
        fill=(0, 0, 0),
        width=line_width,
    )
    draw.text(
...
def stitchTiles(x1, x2, y1, y2, ZOOM, tiles_dir, slope=False):
        ...
        for x in range(x1, x2 + 1):
            if slope:
                tile_filename = os.path.join(
                    tiles_dir, f"slope_tile_{ZOOM}_{x}_{y}.png"
                )
            else:
    ...
    tile_size, _ = Image.open(
        os.path.join(tiles_dir, f"map_tile_{ZOOM}_{x1}_{y1}.png")
    ).size
...
def cropBorders(
    ...
    left_crop = int(
        (
            ECF
            * math.cos(math.radians(northwest_latitude))
            * abs(lon1 - northwest_longitude)
            / 360
        )
        / s_pixel
    )
                ...
                for attempt in range(3):  # Try up to 3 times
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            print(
                                f"Failed to download {tile_url}: {response.status_code}"
                            )
...
def get_icons(coordinates, symbol="firepit"):
    ...
    try:
        ...
        if response.status_code != 200:
            print(
                f"Overpass API returned status {response.status_code}: {response.text}"
            )
            return []
